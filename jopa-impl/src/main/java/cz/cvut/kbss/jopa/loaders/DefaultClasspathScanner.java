...
public class DefaultClasspathScanner implements ClasspathScanner {
    ...
    protected void processClass(String className) {
        try {
            ...
            LOG.debug("Unable to load class {}, got error {}: {}. Skipping the class. If it is an entity class, ensure it is available on classpath and is built with supported Java version.", className, e.getClass().getName(), e.getMessage());
        }
    }
